...
Resolution asset for [ticket #43](https://github.com/philipbergman6-glitch/agents/issues/43) (map #42).
Evidence base for the validation-layer ticket (#44): what the engine's numbers actually
...
1. **market_cap — yfinance, no cross-check, no alternative.** Hard-fails honestly when
   ...
   to notice. Confirms ticket #45's premise; note a shares×price rebuild has EDGAR share
...
1. **Stitched-TTM warnings never reach the diagnosis.** The `net_income_warning` lands in
   ...
   doesn't. (#44: promote to a diagnosis flag, or a validation severity tier.)
2. **NVDA: the share-outlier filter dropped the truth and kept the artifact.** The 3×-median
   ...
   which cohort is corrupt. (#44: split-aware normalization, not majority-cohort exclusion.)
3. **Banks are out-of-model and the failure mode is one tag away from silent.** JPM misses
   ...
   inapplicable rubric wearing a low score, not an error. (#44: detect financial-sector
...
5. **Growth-capex heuristic can push owner earnings negative and it feeds the DCF as-is.**
   ...
   intrinsic value **−$168M** → MoS −1.02 → bearish. Deterministic and faithful to the
   reference, but a negative-intrinsic-value output is presented with the same confidence
   arithmetic as any other. (#44/audit consumer: at minimum flag OE<0 explicitly.)
...
- **CAVA real growth reads as anomaly**: NI ×3.06 window-over-window and ×9.81
  ...
  that scale-jump checks in #44 need a legit-hypergrowth escape hatch.
...
## What this means for the validation layer (#44)
...
2. Market-cap cross-check vs EDGAR shares × price once #45 lands its price source; until
