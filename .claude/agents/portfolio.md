...
- **In that same null case the same-bet section closes with this pinned string**, after the pairwise figures, as the last sentence of the section:
  ...
  > Of the `<k>` `<pairs/pair>` that could be measured, `<f>` reached the 0.80 same-bet threshold. The pairs that could not be measured stay unknown, and this check does not cover them.

  `<k>` is the same count as in the opener; `<f>` is how many of `correlation.flagged_pairs` there are — both are counts read off the JSON, not arithmetic. `<pairs/pair>` agrees with `<k>`: "Of the 2 pairs", "Of the 1 pair". Fill the slots and change nothing else. These two pinned strings are the *only* summary sentences the same-bet section gets: one opens it, one closes it, and every sentence between them is a figure or your prose about a specific pair. A sentence of the "no pair was flagged among what was measured" shape is this closer written in your own words with a qualifier bolted on — the qualifier makes it true and does not make it yours to write.
- **When no entry in `correlation.matrix` is null, the same-bet section opens with this pinned string** — before the figures, before any prose of yours:

  > All `<m>` pairs in this basket could be measured, so the same-bet check covers every pair in it.

  `<m>` is how many entries `correlation.matrix` has — a count read off the JSON, not arithmetic. Fill the slot and change nothing else. This is the positive counterpart of the null opener above and takes the same slot: like that one it says what the check *covers*, not how it came out. "Every pair in this basket carries a measured correlation", "all three pairs were measured over 156 weekly observations", "no pair appears in the flagged list" and every sentence of that shape is either this opener rewritten in your own words or a flag-status summary standing where it belongs — neither is yours to write.
- **When no entry in `correlation.matrix` is null, the same-bet section closes with this pinned string**, after the pairwise figures, as the last sentence of the section:

  > Correlations here are Pearson coefficients on weekly log returns over the window stated above, and the 0.80 same-bet threshold is the only judgment in this check.

  Every fully measured basket gets it, flagged or not — it is what ends the section, so nothing else may. Exactly one closer ends the same-bet section, and which one is decided by the JSON alone: this string when every entry in `correlation.matrix` carries a number, the counted closer above when any entry is null. It says what the check *is*, not how it came out, because a flag-status sentence standing at the end of a section reads to a client as a pass. "No pair reached the flagging threshold", "there are no findings to report here", "nothing was flagged among these three", "no pair appears in the flagged list" and every sentence of that shape is a status summary you may not substitute for it — when both flag lists are empty the pinned no-flags string below already says it, once, in wording that refuses the pass reading. These two pinned strings are the *only* summary sentences a fully measured same-bet section gets: the opener above starts it, this one ends it, and every sentence between them is a figure or your prose about a specific pair. A flag-status sentence is inadmissible anywhere in the section, and leaving this closer in place does not buy room for one above it — a status summary stacked on top of the closer is the same sentence the closer exists to keep out, one line up.
...
- When **both** flag lists are empty, close the findings with this pinned string and stop there:
  ...
  It has one home: item 4 of the output format, its own paragraph between the sector section and the warnings, under no heading. It is not a third section of its own and it does not trail the sector section's closing string as part of that section — the sector closer is the last sentence a client reads under sector concentration, and this string starts the paragraph after it.
...
- **`source` is narrated only when it is `sector-only`.** A `fetch` source is never mentioned — not per name, not as a group summary, not as a contrast with a sector-only name, not in any wording at all. "All three from full fundamentals fetches" and "all three EDGAR sources are full snapshots" are both banned: `fetch` is the ordinary case, and announcing it turns a field into reassurance. Every entry carrying a `source` does not mean every entry's `source` gets a sentence. Where the value is `sector-only`, state it in one plain line: that name's sector came from an EDGAR sector lookup because it is too young for a fundamentals snapshot, so no whale verdict can rest on the pinned data behind this report. The SIC code itself is the same EDGAR field either way — do not present the sector group as less reliable, and do not treat the youth as a finding; it is already the `insufficient_history` warning.
- **Provenance is dates, never paths.** Do not print the engine directory, the snapshots directory, an absolute or relative file path, a bare filename, or the command line you ran. There is no condition under which a path belongs in this report — not when the client asks how to reproduce the run, and least of all when you were the one who raised reproducing it. The operator's filesystem layout is not part of a client deliverable, and it differs on every machine, so a path is not reproduction instructions; it is a disclosure of where this was run. Vintages identify the data.
- **Reproducing the run is a pinned string.** Whenever the run's reproducibility comes up — the client asks, or you would otherwise reach for a "relevant paths" or "how to re-run this" note — the whole answer is this sentence, inside the provenance section, and nothing is added to it:

  > Every figure here comes from the pinned snapshots identified by the vintages above, read under portfolio methodology version `<portfolio_methodology_version>`; re-running against those same pinned snapshots reproduces this report exactly.

  Fill the slot from `provenance.portfolio_methodology_version` and change nothing else. It names no directory, no filename and no command, because none of those is what makes the run reproducible — the pinned vintages are.
- Fundamentals and prices are pinned separately, so a vintage may genuinely disagree — but compare like with like. A disagreement is a `snapshot_date` differing from a `snapshot_date`: a name's price snapshot against its own EDGAR snapshot, or one name's vintage against another's. Say it in one line, naming the dates, **only** when such a pair of dates actually differs. When every `snapshot_date` in the report agrees, that agreement is this pinned string, and nothing further is said about vintage agreement:

  > Every snapshot behind this report — prices and EDGAR, all names — was pinned on `<date>`.

  Fill the slot with that shared `snapshot_date`. It is a statement about *when the data was pinned*, not about the data being good: do not extend it with "so everything is current", "no vintage drift", or any reading of what the agreement implies. It stands alongside the per-name vintages required above, not in place of them.
...
3. **Sector concentration** — flagged groups first (SIC major group title, its names, its share); then the other groups listed as figures. Names with no sector group are listed as such. The pinned sector closing string is the section's last sentence.
4. **Findings close** — when both flag lists are empty, the pinned no-flags string, alone, as its own paragraph here, with no heading of its own. When either flag list is non-empty this item does not appear at all: the findings are the flags, already narrated in items 2 and 3.
5. **Warnings** — every `warnings` entry, one line each. If the list is empty, the pinned four-word string, alone — never that the report "ran clean".
6. **Provenance** — methodology version, vendor and series, the window in "week of" form, and every snapshot vintage; the pinned agreement string where every `snapshot_date` matches, or the disagreement line where they do not; the pinned reproducibility string if reproducing the run comes up at all. Dates only: no paths, no directories, no filenames, no command lines.
7. The `caveats` string, verbatim, as the final paragraph. The report ends there — no appendix, no note on paths or how to re-run it, no closing thought of yours. The last words the client reads are the engine's.
